$(document).ready(function() {
	...
	$('body').delegate('.operate-delete', 'click', function() {
		...
		$.ajax({
			type : 'post',
			url : BASE_URL + '/activitySeckill/delete',
			data : 'id=' + id,
			dataType : 'json',
			timeout : 10000,
...
function formatData(items) {
	$.each(items, function(index, item) {
		item._query = '<a href="'+BASE_URL + '/activitySeckill/detail?id=' + item.id + '"  class="modal-detail"><i class="fa fa-search-plus" title="查看详情"></i></a>';
		item._action = '<a href="' + BASE_URL + '/activitySeckill/edit?id=' + item.id
				+ '" class="operating-edit" title="编辑"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;'
				+ '<a href="javascript:;" class="operate-delete" id="' + item.id
				+ '" title="删除"><i class="fa fa-trash-o"></i></a>';

		item.showTime = dateConverter(item.showTime, PATTERN_ENUM.datetime);
		...
		var strIsWhiteListActivity = '未知';
		if (item.isWhiteListActivity == 0) {
			strIsWhiteListActivity = '不是';
		} else if (item.isWhiteListActivity == 1) {
			strIsWhiteListActivity = '是';
		}
        item.isWhiteListActivity = strIsWhiteListActivity;
